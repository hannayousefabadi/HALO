...
def main():
    ...
    def make_split_cv2(
        ...
        min_frac: float = 0.16,         # keeping 16 to 28 % for test set
        max_frac: float = 0.28,
        lambda_penalty: float = 1.0,    # tie-breaker: score = kept - λ * dropped
        top_k_print: int = 5,
        verbose: bool = True,
    ):
        """
        ...
        # enumerate all non-empty subsets of strains
        candidates = []
        ...
        if not candidates:
            if verbose:
                ...
                print("=" * 72)
            # fall back to empty split (all train) to avoid crashes
        ...
        # rank candidates: least dropped, then most kept, then best score
        ...
        # build final positional indices
        ...
        # sanity check (entity disjointness)
    ...
    print(
        "Train AUC:",
        round(roc_auc_score(y_tr, p_tr), 3),
        "| Test AUC:",
        round(roc_auc_score(y_te, p_te), 3),
    )
    print(
        "Train Acc:",
